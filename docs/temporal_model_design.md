...
- 同一学生数据按 `TRAIN_WINDOWS = (10, 15, 20, 25, 30)` 五个长度滑窗切，每个窗口出
  一个 `(X[3, L], y[5])` 训练对；多窗口分桶让模型见过短/长 context 都不偏科，相比固定
  窗口属于标准数据增强（DA）
- 切窗后每个学生 200 条数据出 161 + 156 + 151 + 146 + 141 = 755 个训练对（合并所有窗口），
  乘以 6 学生 = 4530 总样本，按 70/15/15 切后 train=3480 / val=204 / test=204
    ...
    with _model_lock:
        ...
        if cached is None:
            ...
            if pretrained is not None:
                ...
                return pretrained
        # 没有 .pt 或 load 失败时走老的 lazy train + RETRAIN_DELTA 重训路径
        ...
```
