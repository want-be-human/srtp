...
`(smooth+width)/2`、`defect·0.6 + smooth·0.4`、`total·0.92`。外部看一眼就知道是
...
**创新点**：雷达图是项目里最容易被外部盯的可视化之一。把"凑数公式"换成"独立可观测"
直接消除外部最常见的质疑。
...
**创新点**：把"焊缝两侧必须是基板"的领域知识硬编进特征筛选——外部追问"为什么不把
反光当焊缝"时有明确答案。
...
**创新点**：外部最常质疑的"你这宽度数字怎么算出来的"——把"标定 → 推理 → 显示
...
1. **训练数据集物理位置**：[`backend/services/prediction/artifacts/temporal_training_data.csv`](../backend/services/prediction/artifacts/temporal_training_data.csv)
   ...
   CSV 落盘外部可直接 Excel/pandas 打开复审。`welding.db` 行数永远等于真实演示
...
本节的离线脚本是**外部追问"训练曲线和指标在哪"的标准答案**，跟线上路径解耦。
...
| [`artifacts/temporal_training_curve.png`](../backend/services/prediction/artifacts/temporal_training_curve.png) | ~30 KB    | 训练曲线，演示 PPT 直接贴                                       |
...
- **手动重训**：跑 `python scripts/train_temporal_offline.py` 拉到 docs 下，把 .pt 文件传到演示机覆盖即可，不用重新跑训练
...
- 配合 E-P1-1 形成"图像预处理 → 焊缝定位 → 纹理评分"三段流水线，对外能讲出
  清晰的算法 pipeline
...
P0-P3 11 项完成后做了一轮全栈审计，发现以下「已实现但展示不到位 / 外部可攻破」的弱点。
...
E-P3-1 缺陷热图、综合分、雷达 6 维都依赖 DB 历史记录。为演示时拿"演示数据当真实数据用"，
...
除 DB 外 5 处 mock，全部在 UI 上标识，让外部一眼看清「哪是真 / 哪是兜底」：
