...
# 每多 1 条新记录就重算预测；TTL 60s 兜底防止长会话拿陈旧结果
PREDICTION_CACHE_THRESHOLD = 1
...
def _get_detection_data_from_db(db, limit: int = PREDICTION_HISTORY_LIMIT, student_id: Optional[str] = None) -> list:
    ...
    for record in reversed(records):
        data.append({
            ...
            'width_score': record.spacing_score or 0,
            'defect_score': record.defect_type_score or 0,
            'defect_type_name': record.defect_type_name or '未知',
...
def _generate_demo_data(days: int = 15) -> list:
    """数据库为空时返回的占位数据，标 data_source=DEMO 给前端做兜底提示。"""
    import random
...
async def get_prediction(
    ...
    try:
        ...
        logger.info(f"使用最近 {len(historical_data)} 条数据进行预测")
        ...
        logger.info(f"预测完成 mode={mode}，历史 {len(prediction_result['history'])} / 预测 {len(prediction_result['forecast'])}")
        ...
        if detection_data:
            ...
            if total_defects > 0:
                defect_stats = {
                    ...
                    for key, count in actual_defects.items() if count > 0
                }
            else:
                defect_stats = {
            ...
            defect_stats = {defect_type: 0 for defect_type in TRUE_DEFECT_TYPES[:6]}
...
async def custom_prediction(
    ...
    try:
        ...
        # 不在这个端点生成图表，前端需要图表走 /predict/charts-only
        return {
            "history": prediction_result['history'],
            "forecast": prediction_result['forecast'],
...
async def get_charts_only():
    """返回 base64 编码的折线 + 双雷达图，前端拿来直接 src 用。"""
    try:
        import importlib
        try:
            data_gen = importlib.import_module('data_generator')
...
async def analyze_scores_with_ai(scores_data: Dict[str, float]):
    """传入 total/smoothness/width/defect 四项 score，AI 给出针对当次的建议。"""
    try:
        if ai_service:
...
async def get_custom_ai_prediction_analysis(request_data: Dict[str, Any]):
    """用户自带 historical_data + current_data 跑 AI 分析，不查 DB。"""
    try:
...
async def receive_yolo_detection_data(yolo_data: YOLODetectionData, db: Session = Depends(get_db)):
    """前端推 YOLO 实时检测数据进来直接落库。"""
    try:
        # 兼容两种字段命名：带 _score 后缀和不带后缀的
        detection_data = {
...
def _aggregate_radar(records):
    """把一批焊接记录算成两套雷达图维度：defect_radar 缺陷类型分布、skill_radar 技能 6 维。"""
